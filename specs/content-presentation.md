...
- Transcript text is read-only, selectable, copyable, and divided into timestamped segments. Preview and transient presentations use a subordinate bounded, collapsible panel that never obscures essential controls or makes video unusably small.
- Details places the complete expanded transcript last, after copies and companions and a separator. It uses Details' normal scrolling rather than a separate short transcript viewport. Its visibility and reading position do not change Preview's transcript state.
- Video and audio remember Preview transcript open/collapsed state independently. Preview placement and transient-presentation switches retain panel state, scroll, and text selection for the same live media session. Restart persistence of that preference is not specified.
