...
import { backgroundRows, installDerivedWorkEventWiring, useDerivedWorkStore } from "../../src/state/derived-work-store";
...
it("refreshes newly discovered debt without an open modal and preserves newer running state", async () => {
  ...
  mockCommands({ background_work_snapshot: () => ({ pausedClasses: [], classes: [row], activeItem: null }) });
  ...
  finish({ pausedClasses: [], classes: [{ ...row, state: "queued", queued: 20 }], activeItem: null });
