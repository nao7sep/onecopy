...
describe("the footer's managed-tools chip", () => {
  it("says nothing when everything installed is fine", () => {
    expect(toolsChip(null, [entry("ffmpeg", "up-to-date")])).toBeNull();
  });
  ...
  it("shows a calm, normal-ink line when installed but never checked", () => {
    ...
    const chip = toolsChip(null, [entry("ffmpeg", "installed-unchecked")]);
    expect(inEnglish(chip?.text)).toBe("Tools not checked");
    expect(chip?.role).toBe("neutral");
  });
  ...
  it("names the unreadable case, which only re-acquiring can clear", () => {
    ...
    const chip = toolsChip(null, [unreadable]);
    expect(inEnglish(chip?.text)).toBe("Tool version unreadable");
    expect(chip?.role).toBe("neutral");
  });

  it("keeps absent optional models off the chip", () => {
    expect(
      toolsChip(null, [entry("ffmpeg", "up-to-date"), entry("whisper-large-v3-turbo", "not-installed")]),
    ).toBeNull();
  });

  it("says nothing before the state has loaded", () => {
    expect(toolsChip(null, [])).toBeNull();
  });
  ...
  it("warns with remedy-shaped copy when ffmpeg is missing", () => {
    ...
    const chip = toolsChip(null, [entry("ffmpeg", "not-installed")]);
    expect(chip?.role).toBe("warning");
    expect(inEnglish(chip?.text)).toMatch(/video/i);
    expect(inEnglish(chip?.text)).toMatch(/HEIC/);
  });
  ...
  it("warns when any entry has an update, counting them honestly", () => {
    const one = toolsChip(null, [
      entry("ffmpeg", "up-to-date"),
      entry("whisper-large-v3-turbo", "update-available"),
    ]);
    expect(one?.role).toBe("warning");
    expect(inEnglish(one?.text)).toBe("Tool update available");
    const two = toolsChip(null, [
      entry("ffmpeg", "update-available"),
      entry("whisper-large-v3-turbo", "update-available"),
    ]);
    expect(inEnglish(two?.text)).toBe("Tool updates available");
  });

  it("shows progress while installing, whatever the stored states", () => {
    const progress = message("install.downloading", { bytes: "4 MB / 10 MB (40%)" });
    const chip = toolsChip(progress, [entry("ffmpeg", "not-installed")]);
    expect(chip).toEqual({ text: progress, role: "neutral" });
  });
  ...
  it("never puts a version number in the footer", () => {
    ...
    for (const status of statuses) {
      const chip = toolsChip(null, [entry("ffmpeg", status)]);
      if (chip !== null) expect(inEnglish(chip.text)).not.toMatch(/\d+\.\d+/);
    }
  });
});
